...
LiveValidation.prototype = {
    ...
    initialize: function(d, c) {
        ...
        var b = c || {};
        this.validMessage = b.validMessage || b.validMessage !== false && LiveValidation.OPTIONS.validMessage;
        var e = b.insertAfterWhatNode || LiveValidation.OPTIONS.insertAfterWhatNode || this.element;
        this.insertAfterWhatNode = e.nodeType ? e : document.getElementById(e);
        this.onlyOnBlur = b.onlyOnBlur || LiveValidation.OPTIONS.onlyOnBlur;
        this.wait = b.wait || LiveValidation.OPTIONS.wait;
        this.onlyOnSubmit = b.onlyOnSubmit || LiveValidation.OPTIONS.onlyOnSubmit;
        this.beforeValidation = b.beforeValidation || LiveValidation.OPTIONS.beforeValidation || function() {};
        this.beforeValid = b.beforeValid || LiveValidation.OPTIONS.beforeValid || function() {};
        this.onValid = b.onValid || LiveValidation.OPTIONS.onValid;
        this.afterValid = b.afterValid || LiveValidation.OPTIONS.afterValid || function() {};
        this.beforeInvalid = b.beforeInvalid || LiveValidation.OPTIONS.beforeInvalid || function() {};
        this.onInvalid = b.onInvalid || LiveValidation.OPTIONS.onInvalid;
        this.afterInvalid = b.afterInvalid || LiveValidation.OPTIONS.afterInvalid || function() {};
        this.afterValidation = b.afterValidation || LiveValidation.OPTIONS.afterValidation || function() {};
        if (this.form) {
        ...
        if (this.elementType == LiveValidation.RADIOGROUP) {
            var f = this;
            $(this.element).children("input[type=radio]").on("focus", function() {
        ...
        this.element.onfocus = function(g) {
            a.doOnFocus(g);
            return a.oldOnFocus.call(this, g)
        };
        if (!this.onlyOnSubmit) {
            switch (this.elementType) {
                case LiveValidation.CHECKBOX:
                    this.element.onclick = function(g) {
                        a.validate();
                        return a.oldOnClick.call(this, g)
                    };
                case LiveValidation.SELECT:
                case LiveValidation.FILE:
                    this.element.onchange = function(g) {
                        a.validate();
                        return a.oldOnChange.call(this, g)
                    };
                    break;
                default:
                    if (!this.onlyOnBlur) {
                        this.element.onkeyup = function(g) {
                            a.deferValidation();
                            return a.oldOnKeyup.call(this, g)
                        }
                    }
                    this.element.onblur = function(g) {
                        a.doOnBlur(g);
                        return a.oldOnBlur.call(this, g)
                    }
            }
        }
    },
    ...
    remove: function(b, e) {
        var f = [];
        for (var d = 0, a = this.validations.length; d < a; d++) {
            var c = this.validations[d];
            if (c.type != b && c.params != e) {
                f.push(c)
            }
        }
        this.validations = f;
    ...
    getElementType: function() {
        var b = this.element.nodeName.toUpperCase();
        if ($(this.element).hasClass("radiogroup")) {
            var a = "RADIOGROUP"
        } else {
            var a = this.element.type.toUpperCase()
        }
        switch (true) {
            case (b == "TEXTAREA"):
            ...
            case (b == "INPUT" && (a == "EMAIL" || a == "URL" || a == "TEL" || a == "NUMBER" || a == "RANGE")):
                return LiveValidation.TEXT;
            case (b == "INPUT" && a == "PASSWORD"):
                return LiveValidation.PASSWORD;
            case (b == "INPUT" && a == "CHECKBOX"):
                return LiveValidation.CHECKBOX;
            case (b == "INPUT" && a == "FILE"):
                return LiveValidation.FILE;
            case (b == "DIV" && a == "RADIOGROUP"):
                return LiveValidation.RADIOGROUP;
            case (b == "SELECT"):
                return LiveValidation.SELECT;
            case (b == "INPUT"):
                throw new Error("LiveValidation::getElementType - Cannot use LiveValidation on an " + a.toLowerCase() + " input!");
            default:
                throw new Error("LiveValidation::getElementType - Element must be an input, select, or textarea - " + b.toLowerCase() + " was given!")
        }
    },
    doValidations: function(d) {
        var d = d || function(e) {
            return e
        };
        this.validationFailed = false;
        var c = this;
        var a = function(e, g, h) {
            if (e.length == 0) {
                c.validationFailed = g;
                c.message = c.validMessage;
                return h(g)
            } else {
                var f = e.shift();
                return c.validateElement(f.type, f.params, function(j) {
                    var i = g;
                    if (!j) {
                        i = false;
                        return h(i)
        ...
        var b = this.validations.slice(0);
        return a(b, true, d)
    },
    validateElement: function(a, c, f) {
        var f = f || function(g) {
            return g
        };
        switch (a) {
        ...
        if (this.elementType == LiveValidation.SELECT) {
            if (this.element.selectedIndex >= 0) {
                d = this.element.options[this.element.selectedIndex].value
            }
        } else {
            if (this.elementType == LiveValidation.RADIOGROUP) {
                d = $(this.element).children("input[type=radio]:checked").val()
            } else {
    ...
    validate: function(b) {
        ...
        if (!this.element.disabled) {
            ...
            var a = this;
            return this.doValidations(function(c) {
                if (c) {
                    a.beforeValid();
                    a.onValid();
                    a.afterValid();
    ...
    insertMessage: function(c) {
        ...
        if ((this.displayMessageWhenEmpty && (this.elementType == LiveValidation.CHECKBOX || this.element.value == "")) || this.element.value != "") {
            var b = this.validationFailed ? this.invalidClass : this.validClass;
            c.className += " " + this.messageClass + " " + b;
            var a = this.insertAfterWhatNode.parentNode;
            if (this.insertAfterWhatNode.nextSibling) {
                a.insertBefore(c, this.insertAfterWhatNode.nextSibling)
            } else {
                a.appendChild(c)
            }
        }
    ...
    removeFieldClass: function() {
        var a = this.element.className;
        if (a.indexOf(this.invalidFieldClass) != -1) {
            this.element.className = a.split(this.invalidFieldClass).join("")
        }
        if (a.indexOf(this.validFieldClass) != -1) {
            this.element.className = a.split(this.validFieldClass).join(" ")
        }
    },
...
LiveValidationForm.getInstance = function(a) {
    ...
    var c = a.nodeName ? a : document.getElementById(a);
    var b = Math.random() * Math.random();
    if (!c.id) {
        c.id = "formId_" + b.toString().replace(/\./, "") + new Date().valueOf()
    }
    if (!LiveValidationForm.instances[c.id]) {
        LiveValidationForm.instances[c.id] = new LiveValidationForm(c)
    }
    return LiveValidationForm.instances[c.id]
};
LiveValidationForm.prototype = {
    ...
    initialize: function(b) {
        ...
        var a = this;
        this.element.onsubmit = function(d) {
            var c = false;
            a.beforeValidation(), a.valid = LiveValidation.massValidate(a.fields);
            a.valid ? a.onValid() : a.onInvalid();
            a.afterValidation();
            if (a.valid) {
                c = a.oldOnSubmit.call(this, d || window.event) !== false
            }
            if (!c) {
                return c
            }
